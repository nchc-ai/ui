...
<?php
	...
	$dataset_choose=explode(",",$rowcourse['dataset']);
	$dataset_num=count($dataset_choose);
        echo '
	<form method="post" action="course_update.php" id="ctl00">
...
//	echo '			<div class="form-group g-mb-20"><textarea name="course_intro" rows="8" cols="20" id="course_intro" class="form-control" required="" autocomplete="false">'.$rowcourse['course_intro'].'</textarea></div>'."\n";
	echo '			<script type="text/javascript" src="../ckeditor-full/ckeditor.js"></script>
				<div class="form-group g-mb-20"><textarea class="ckeditor" name="course_intro" id="course_intro" rows="8" cols="20" required="" autocomplete="false">'.$rowcourse['course_intro'].'</textarea></div>'."\n";
        echo '                  <div class="form-group g-mb-20" style="font-weight:bold;">Docker Image</div>';
//	echo '			<div class="form-group g-mb-20">'.$post_docker_image_name.":".$post_docker_image_tag.' </div>';
        echo '                  <div class="form-group g-mb-20"><select name="DDL_di_id" id="DDL_di_id" class="form-control">';
	...
	echo '<table>';
	$array = ckan_api_checked($dataset_choose);
	$Count=$array[0];
	$used=$array[1];
//        echo '                  <div class="form-group g-mb-20">'.$rowcourse['dataset'].' </div>';
	...
	$result = mysqli_query($conn, $sql);
//	echo $dataset_choose[0];
	while ($row = mysqli_fetch_assoc($result)) {
		$checked=0;
		for ($j=0;$j<$dataset_num;$j++){
                        if ($row['name'] == $dataset_choose[$j]){
        echo '<tr><td><input type="checkbox" name="DSet'.$Count.'" id="DSet'.$Count.'" value="'.$row['name'].'" checked><label>'.$row['name'].'</label</td></tr>'."\n";
        $checked=1;
			}
                }
        if ($checked!=1){
        echo '<tr><td><input type="checkbox" name="DSet'.$Count.'" id="DSet'.$Count.'" value="'.$row['name'].'"><label>'.$row['name'].'</label</td></tr>'."\n";
	}
        $Count+=1;
	}
	echo '</table>';
	echo '<input type="hidden" id="DSetCount" name="DSetCount"  value="'.$Count.'">'."\n";
	echo '<input type="hidden" id="update_time" name="update_time"  value="'.$rowcourse['update_time'].'">'."\n";
        echo '
			</div>
		</div>
	</section>';
...
<?php
//echo '<form method="post" action="course_del.php">';
//echo '<br />';
//echo '<input type="hidden" name="course_id" value="'.$rowcourse['id'].'">';
//echo '<button type="submit" value="sub" style="background-color:rgb(255,0,0);color:white;">刪除</button>';
?>
